...
export function Choice({ variant = "radio", selected, onClick, letter, children }: ChoiceProps) {
  return (
    <button
      ...
      onClick={onClick}
      className={`w-full flex items-center gap-3 text-left rounded-xl border p-3 min-h-[46px] text-sm transition
    ...
    >
      <span
        aria-hidden="true"
        className={`shrink-0 w-5 h-5 flex items-center justify-center text-[11px] border transition
...
export function SegChoice({ selected, onClick, children }: { selected: boolean; onClick: () => void; children: ReactNode }) {
  return (
    <button
      ...
      aria-checked={selected}
      onClick={onClick}
      className={`flex-1 min-w-[110px] text-center rounded-lg border p-2 min-h-[36px] text-xs transition
        focus:outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#286575] focus-visible:ring-offset-1
