    ...
    <div className="min-h-screen bg-[#f3fbfb]">
      <div className="bg-white/10 border-b border-white/20 rounded-b-2xl">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-2.5 flex items-center gap-3">
          <Link href="/dashboard"><img src="/ladico_green.png" alt="Ladico" className="w-11 h-11 object-contain cursor-pointer hover:opacity-80" /></Link>
          <span className="text-[#2e6372] text-xs sm:text-sm opacity-80 bg-white/10 px-3 py-1 rounded-full">{label}</span>
        </div>
      </div>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-2.5">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs text-[#286575] font-medium bg-white/10 px-3 py-1 rounded-full">Pregunta {index} de {total}</span>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pb-6">
        <Card className="bg-white shadow-xl rounded-2xl border-0 ring-1 ring-[#286575]/20">
          <CardContent className="p-4 sm:p-5">
            ...
            {instruction && open && (
              <div id="exercise-instruction" className="mt-2 mb-3 bg-gray-50 p-3 rounded-xl border-l-4 border-[#286575]">
                <p className="text-gray-700 whitespace-pre-line leading-snug text-[13px] sm:text-sm">{instruction}</p>
              </div>
            )}
            <div className="mt-3">{children}</div>
            <div className="mt-4 flex justify-end">
              <Button onClick={onNext} disabled={nextDisabled} className="px-8 sm:px-10 py-2.5 bg-[#286575] rounded-xl font-medium text-white shadow-lg hover:bg-[#3a7d89] disabled:opacity-50 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-[#286575] focus-visible:ring-offset-2">{nextLabel}</Button>
            </div>
          </CardContent>
        </Card>
      </div>
