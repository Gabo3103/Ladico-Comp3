...
function ResultsIntermedioContent() {
    ...
    const fallback = useMemo(() => {
        // Clave usada por setPoint(COMPETENCE="3.4", LEVEL="intermedio", idx, 0|1)
        const key = "ladico:3.4:intermedio:progress"
        try {
        const raw = typeof window !== "undefined" ? localStorage.getItem(key) : null
        const arr: number[] | null = raw ? JSON.parse(raw) : null // p.ej. [1,0,1]
    ...
    const data = useMemo(() => {
        // Si vienen por query, usar esos. Si no, usar fallback local.
        ...
        const level = (qp.level || "intermedio").toLowerCase()
        const q1 = qp.q1 === "1"
        const q2 = qp.q2 === "1"
        const q3 = qp.q3 === "1"
        const sid = qp.sid || null
        return { score, passed, correct, total, competence, level, q1, q2, q3, sid }
        }
        // Si no vienen, usar fallback (si existe)
        if (fallback) {
        return { ...fallback, sid: null as string | null }
        }
        // Último recurso: todo a cero
        return {
        score: 0,
        passed: false,
        correct: 0,
    ...
    useEffect(() => {
        ...
        if (!data.sid) return
        try {
            await finalizeSession(data.sid, {
            correctCount: data.correct,
            total: data.total,
            passMin: 2, // aprueba con 2+
            })
        } catch (e) {
